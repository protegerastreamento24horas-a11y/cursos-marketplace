...
function PlatformBadge({ platform }: { platform: Course["platform"] }) {
  const styles: Record<string, string> = {
    kiwify: "bg-green-50 text-green-700 border-green-200",
    hotmart: "bg-orange-50 text-orange-700 border-orange-200",
    eduzz: "bg-blue-50 text-blue-700 border-blue-200",
    braip: "bg-purple-50 text-purple-700 border-purple-200",
  ...
  return (
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map((star) => (
        <svg
          key={star}
          className={`h-3.5 w-3.5 ${star <= Math.round(rating) ? "text-[#FFC107]" : "text-gray-300"}`}
          fill="currentColor"
          viewBox="0 0 20 20"
        >
...
export default function CourseCard({ course, compact }: { course: Course; compact?: boolean }) {
  ...
  return (
    <Link href={link} className="group block">
      <div className="relative bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 hover:-translate-y-1 hover:border-[#FFC107]/30">
        <div className={`relative ${compact ? "aspect-[16/9]" : "aspect-[3/2] sm:aspect-[4/3]"} bg-gray-100 overflow-hidden`}>
          <Image
            src={course.image}
            alt={course.title}
            fill
        ...
        <div className="p-3.5 sm:p-4">
          <div className="flex items-center gap-1.5 mb-2">
            ...
            <span className="text-[10px] text-gray-400 uppercase tracking-wider truncate">{course.category}</span>
          </div>

          <h3 className="text-sm font-bold text-[#1B1F2A] mb-1.5 line-clamp-2 group-hover:text-[#FFC107] transition-colors leading-snug min-h-[2.5rem]">
            {course.title}
          </h3>
