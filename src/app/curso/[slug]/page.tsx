  ...
  return (
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map((star) => (
          <svg
          key={star}
          className={`h-5 w-5 ${star <= Math.round(rating) ? "text-[#FFC107]" : "text-gray-300"}`}
          fill="currentColor"
          viewBox="0 0 20 20"
        >
      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        ...
        <div className="grid lg:grid-cols-[1fr,380px] gap-8 lg:gap-12">
          <div>
            ...
            <div className="flex items-center gap-2 mb-4">
              <span className="inline-flex items-center rounded-md border border-gray-200 bg-gray-50 px-2.5 py-1 text-xs font-semibold text-gray-500 uppercase tracking-wider">
                {course.category}
              </span>
              <span className="inline-flex items-center rounded-md border border-gray-200 bg-gray-50 px-2.5 py-1 text-xs font-semibold text-gray-500 uppercase tracking-wider">
                {course.platform}
            ...
            <h1 className="text-2xl sm:text-3xl font-bold text-[#1B1F2A] mb-4 leading-tight">
              {course.title}
            </h1>
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
              ...
              {course.affiliateLink !== course.checkoutLink && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center justify-center gap-2 w-full mt-3 rounded-xl border border-gray-200 bg-gray-50 px-6 py-3 text-sm font-medium text-gray-500 hover:border-[#FFC107]/30 hover:text-[#FFC107] transition-all"
                >
                  Ver Pagina do Curso
                ...
                <div className="mt-3 space-y-2">
                  {course.additionalLinks.map((link, i) => (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center justify-center gap-2 w-full rounded-xl border border-gray-200 bg-gray-50 px-5 py-2.5 text-xs font-medium text-gray-500 hover:border-[#FFC107]/20 hover:text-[#FFC107]/80 transition-all"
                    >
                      {link.label}
        ...
        {related.length > 0 && (
          <section className="mt-16">
            <h2 className="text-xl font-bold text-[#1B1F2A] mb-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
              {related.map((c) => (
                <Link
                  key={c.id}
                  href={`/curso/${c.slug}`}
                  className="group block bg-white rounded-xl border border-gray-200 overflow-hidden hover:border-[#FFC107]/30 transition-all shadow-sm"
                >
                  ...
                  <div className="p-3">
                    <h3 className="text-xs font-bold text-[#1B1F2A] line-clamp-2 group-hover:text-[#FFC107] transition-colors min-h-[2rem]">
                      {c.title}
                    </h3>
